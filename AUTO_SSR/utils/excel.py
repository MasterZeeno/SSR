...
def openwb(wb_path, read_only=True, data_only=True):
    return load_workbook(wb_path, read_only=read_only, data_only=data_only)

def getws(wb_path, index=-1, visible_only=True, read_only=True, data_only=True):
    wb = openwb(wb_path, read_only=True, data_only=True)  
    shts = [s for s in wb.worksheets if s.sheet_state == "visible"] if visible_only else wb.worksheets
    return (
        shts if str(index).lower() == "all" else
        (
            [shts[i] for i in index]
            if isinstance(index, (list, tuple))
            and i <= len(shts) else shts[index]
        )
    )
    
def extract_data(wb_path):
    ws = getws(wb_path)
    return {
        f"{{{{ {k} }}}}": v
        for k, v in {
            "year_now": str(datetime.now().year),
            "reference_no": getValue(ws,61,2).split()[-1],
            "report_period": fmt_date(getValue(ws,63,4)),
            **{
                f"{r-59}:{c-18}": getValue(ws,r,c)
                for c in range(18, 21) for r in range(59, 68)
            }
        }.items()
    }
    ...
    # getValues(
                # ws=ws,
                # min_row=59, max_row=67,
                # min_col=18, max_col=20
            # )
