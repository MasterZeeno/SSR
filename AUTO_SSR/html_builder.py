...
ZEE = {
    ...
    "link": f"https://facebook.com/{REPO_OWNER}"
}
...
SUBJECT = f"Update: {REPORT[1]} ({REPORT[1]}) for {HEADERS[1][-1]}"
...
WRAPPER = ';'.join([
    f'{LANG_DIR} style="background-color:{BG_DARK}',
    f'width:100%;height:auto;font-family:{FONT_FAMILY}',
    'margin:0;padding:0;border:none;outline:none;">'
])
CONTENT_DIV = ';'.join([
    f'<div {VALIGN};dir:ltr;word-wrap:break-word',
    f'font-family:{FONT_FAMILY};text-align:left',
    f'color:{FG};font-size:{FONT_SIZE}px',
    f'line-height:{LINE_HEIGHT}'
])
CONTAINER_STYLE = ';'.join([
    f'border-radius:{BORDER_RADIUS}px',
    f'font-family:{FONT_FAMILY}',
    f'width:{MAX_WIDTH}px;max-width:{MAX_WIDTH}px',
    'height:auto;background-color:#fff',
    f'margin:{FONT_SIZE * 0.5}px auto',
    f'padding:{FONT_SIZE * 1.5}px'
])
IMG_STYLES = ';'.join([
    f'{VALIGN};color:{FG_VAR};font-size:{FONT_SIZE/3}px',
    'text-decoration:none;outline:none;border:none'
])
    
ASSETS_LINK = urlify(f'https://raw.githubusercontent.com/{REPO_OWNER}/{REPO_NAME}/refs/heads/main/{Path(__file__).parent.name}/assets')
WB_DOWNLOAD_LINK = f'{ASSETS_LINK}/workbooks/{urlify(WB_PATH.name)}'

def make_img(src):
    obj, size, styles = (
        (ZEE, 64, ';border-radius:100%')
        if "zee" in src else (HCC, 32, '')
    )
    
    link = f"{ASSETS_LINK}/images/{urlify(obj["name"])}.png"
        
    return f"""<a href="{urlify(obj["link"])}" width="{size}" target="_blank" 
        {IMG_STYLES}{styles};display:inline-block;width:{size}px;max-width:{size}px;">
        <img src="{link}" alt="{obj["name"]}" {IMG_STYLES}{styles};width:100%"></a>"""
...
def hr(
    ...
    return ';'.join([
        f'<p role="separator" {VALIGN};color:{color};font-size:{size}px'
        f'opacity:{opacity};text-align:{align};">{"&mdash;" * count}</p>'
    ])
...
def bold(text=None):
    ...
    element = f'<span style="{prop}">{text}</span>'
    return prop if text is None else element
...
SUMMARY_HTML = (
    f'<div style="padding:{FONT_SIZE*0.75}px 0;">' +
    f'<table aria-label="{REPORT[0]}" width="100%" border="0" ' +
    f'cellspacing="0" cellpadding="0" {VALIGN};border-collapse:collapse;' +
    f'font-size:{FONT_SIZE*0.8125}px;color:{FG};"><tbody>' +
    ''.join(
        '<tr>' + ''.join(
            ...
            ])}">{("&nbsp;"*3 + cell if i>0 and j<1 else cell)}</{"th" if i<1 else "td"}>'
