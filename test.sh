...
send_msg() {
    ...
    if [[ -t 1 ]]; then
        ...
        eval "$({
            printf '%s=\\e[0%s\n' esc '' reset m \
                prm "$prm" ylw "$ylw" fnt "$fnt" wht "$wht" | sed -E '/^[^er]/s/(e\[0)(.*)/\1;38;2;\2m/'
            eval "$(printf 'prm=%s\n' ylw fnt | sed -E 'p;s/(.*)=(.*)/\2=\1/' | sed -E -e 's|(.)(.*)=(.)(.*)|\1\3=\${esc};%s;\${\3\4:4:-1};4\${\1\2:5}|' \
                -e "s/.*/printf '\%s&\\\n' n 0 b 1 d 2 i 3/")"
            printf '${%s}\\UE0C\n' ylw prm npy npf | sed -E 's/(\$\{)(.*)(\}.*)/l_\2=\1\2\32/;p;s/^l(.*)2/r\10/'
        ...
        printf "%s\n" "${msgs[@]}" | sed -E "s/^$/\n/;t
          ...
          2,\${s/.*/\n$botsp&/;s/([[:alpha:] ]+):/${wht/0;/1;}\1$wht:/;s/.*/$wht&/}
            \$s/$/$reset\n/"
    else
        printf "%s\n" "${msgs[@]}"
    fi
...
setup_email() {
    ...
    create_file() {
        ...
        if [[ ! -s $file ]]; then
            if [[ $file =~ gpg$ ]]; then
                ...
                printf "$pre%s$del%s\n" "${contents[@]}" | awk -v d="$del" '{split($0,s,d);if(s[2]==""){print s[1];next}
    ...
    eval "$(printf 'create_file "$HOME/.%s"\n' 'gpass.gpg' 'mutt' 'msmtp' | sed -E '/\.m/s/"$/rc&/;/(prc|g)"$/{p;s/.*\s//;s|.*/\.(.*)[."]|\1_file=&|
...
send_email() {
    ...
    local sps=$(spacer 8)
    eval "$(echo '${sps}' | sed -E 'p;h;G;G;G;s/\n/ /g;s/ /:/;s/(([^}]+\}){1})([^}]+)/\1\3:5/
            s/(.*) (.*)$/\1\2/;s/^/To/;s|\}$|:2&/g"|;s| |$(sed "s/ /,\\\\n|
            s/$/ <<<"${recipients[*]}")/;p;s/To/CC/;s/recipients/copies/' | sed -E '/^[^A-Z]/{s/.$/:5&/;h;G;s/\n/:/;s/^/Subject/;s/$/SSR: $REPORT_DATE/
            p;s/5/2/;s/Subject/Date/;s/SSR.*/$(date +"%B %-d, %Y, %-l:%M %p")/}' | sed -E 's/.*/ "&"/;1s/^/details=(\n/;$s/$/\n)/')"
...
[[ -s $SSR_FILE ]] && SSR_FILE=$(realpath "$SSR_FILE") || abort "Invalid excel file: '$SSR_FILE'" \
...
if ((DRY_RUN)); then
    ...
    REPORT_DATE=$(xlsx2csv -a --exclude_hidden_sheets "$SSR_FILE" | grep -Eo '^[-]+.*' | tail -n1 | cut -d ' ' -f4- | perl -pe 's/\b(\w+)\s+(\d+)-\1\s+(\d+),/$1 $2-$3,/;s/\b(\w)/\u$1/g' | sed -E 's/\s+$//')
fi
