...
ZEE = (lambda r: dict(zip(['name', 'position', 'licenses', 'link'],
    [*r[:1], *r[1].split('|'), r[2]])))(
        ...
        if (res := requests.get(f'https://api.github.com/users/{REPO_OWNER}')).ok
        else [
...
SRC_IMG_LINK = f"""https://raw.githubusercontent.com/{REPO_NAME}/{REPO_NAME}/refs/heads/main/{os.path.basename(os.getcwd())}/assets"""
...
def make_img(src):
    ...
    src_link = f"{SRC_IMG_LINK}/{filename}.png"
        
    return f"""<a href="{href}" width="{size}"
...
# SUMMARY_HTML = f"""
# <div style="padding:{FONT_SIZE*0.75}px 0;">
    # <table width="100%" role="table" border="0" cellspacing="0" cellpadding="0" 
        # {VALIGN};border-collapse:collapse;width:100%;font-size:{FONT_SIZE*0.8125}px;color:{FG};">
        # <tbody>
# """
# def_styles = f"""{VALIGN};padding:{FONT_SIZE*0.1875}px {FONT_SIZE*0.375}px;border:{FONT_SIZE*0.0105625}px solid {FG};"""
# for i, row in enumerate(SUMMARY):
    # row_html, tag = "<tr>", "th" if i == 0 else "td"
    # for j, cell in enumerate(row):
        # fs, b = '', bold() if i > 7 and j == 3 else ''
        # align = f"text-align:{'center' if i == 0 or j > 0 else 'left'};"
        # bg = f"background-color:{BG};" if i == 0 or (i > 7 and j == 3) else ''
        
        # if i > 0 and j == 0:
            # fs = "font-style:italic;"
            # cell = "&nbsp;" * 3 + cell
            
        # cell_html = f"""<{tag} align="{align}" 
            # {def_styles}{align}{bg}{fs}{b}">{cell}</{tag}>"""
            
        # row_html += cell_html
        
    # SUMMARY_HTML += row_html + "</tr>"
...
SUMMARY_HTML = (
    ...
    ''.join(
        '<tr>' + ''.join(
            ...
            ])}">{("&nbsp;"*3 + str(cell)) if i>0 and j<1 else cell}</{"th" if i<1 else "td"}>'
            for j, cell in enumerate(row)
        ) + '</tr>'
        for i, row in enumerate(SUMMARY)
    ) + '</tbody></table></div>'
