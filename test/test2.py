...
def make_table(width='100%', bg='transparent', border_radius=0):
    return f'<table role="presentation" align="center" valign="middle" border="0" cellspacing="0" cellpadding="0" width="{re.sub(r'\D+', '', width)}" height="auto" style="border-collapse:collapse;box-sizing:border-box;margin:0 auto;padding:0;border-radius:{border_radius}px;width:{width};max-width:{width};min-width:{width};background-color:{bg};font-family:{FONT_FAMILY};">'
...
def make_cell(
    ...
    ret_val = f'<{el} width="{width.replace('px', '')}" align="{align}" valign="{valign}" style="width:{width}box-sizing:border-box;margin:{margin};padding:{padding};background-color:{bg};text-align:{align};vertical-align:{valign};outline:{outline};border:{border};border-radius:{border_radius};font-size:{font_size}px;font-weight:{font_weight};color:#{font_color};font-style:{font_style};line-height:{line_height}em;word-wrap:break-word;">{value}'
...
signature = f'<tr>{make_cell(parse=False, padding='0 0 30px 0', value=f"{make_table()}<tbody><tr>{make_cell(parse=False, width='64px', value=f'{make_img(img='cimacio', href='linkedin/in/masterzeeno', alt='Jay Ar Cimacio, RN', display='inline-block', width='100%', border_radius='100%')}')}{zee_details}</tr></tbody></table>")}</tr>'
...
{'\n'.join([
    f'{"    "*i}<{el} lang="en" dir="ltr" style="margin:0;padding:0;outline:none;border:none;box-sizing:border-box;width:100%;height:auto;background-color:#{BG_COLOR2};font-family:{FONT_FAMILY};font-size:16px;color:#{COLOR1};font-weight:400;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;{"padding-top:8px;" if el == "div" else ""}">'
    for i, el in enumerate(['body', 'div'])
])}
{'\n'.join([
    f'{" " * 8}<div aria-hidden="true" style="height:0;max-height:0;width:0;max-width:0;overflow:hidden;opacity:0">{v}</div>'
    for v in ('NSB-P2 SSR', '&nbsp;' * 169)
])}
{' ' * 8}{make_table(width='512px', bg='#fff', border_radius=6)}
{' ' * 12}<tbody>
{' ' * 16}<tr>
{' ' * 20}{make_cell()}
{' ' * 24}{make_table()}
{' ' * 28}<tbody>
{' ' * 32}<tr>
{' ' * 36}{make_cell(padding='24px')}
{' ' * 40}{make_table()}
{' ' * 44}<tbody>
{' ' * 48}<tr>
{' ' * 52}{make_cell(padding='24px')}
{' ' * 56}<div>
{' ' * 60}{make_table()}
{' ' * 64}<tbody>
{' ' * 68}<tr>
...
{' ' * 68}</tr>
{' ' * 68}<tr>
{' ' * 72}{make_cell(padding='16px 0 0', font_size=22, font_weight=600, value='Safety Statistics Report (SSR)')}
{' ' * 68}</tr>
{' ' * 68}<tr>
{' ' * 72}{make_cell(padding='0 0 30px', font_color=COLOR3, value='Construction of the New Senate Building (Phase II){{br}}Navy Village, Fort Bonifacio, Taguig City, Philippines')}
{' ' * 68}</tr>
{' ' * 68}<tr>
{' ' * 72}{make_cell(padding='0 0 16px', value='{{b}}Greetings! ✨{{/b}}{{br2}}Please see attached file for the above-mentioned subject.{{br2}}For your convenience, a brief summary is also provided in the table below.')}
{' ' * 68}</tr>
{' ' * 68}<tr>
{' ' * 68}{make_cell(value=f"{table_rows + '</table>'}", parse=False)}
{' ' * 68}</tr>
{' ' * 68}<tr>
{' ' * 72}{make_cell(value='{{br}}Thank you—and as always, {{b}}Safety First! 👊{{/b}}{{br2}}Best regards,{{br2}}')}
{' ' * 68}</tr>
{' ' * 68}{signature}
{' ' * 64}</tbody>
{' ' * 60}</table>
